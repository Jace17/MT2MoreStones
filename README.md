# MT2 More Upgrade Stones

Support language: English | [简体中文](https://github.com/Jace17/MT2MoreStones/blob/main/README_zh.md)

A mod for Monster Train 2 that adds several new upgrade options for cards.

# Features

- Adds 66 new upgrade stones with their own icons, scroll down below see the whole list
- Configurable to disable any new stones you don't want to see in-game (everything is enabled by default)
- Enhanced versions of the new stones for Soul Savior mode

# Installation

Install via Thunderstore or manually: copy the mod contents to the 'BepInEx/plugins' folder of your Monster Train 2 installation directory.

# Warning

This is a non-cosmetic mod that changes the game mechanics. Do not use it while playing any online mode as it may cause you to be banned.

# Suggested Mods

- [Unlimited Purchases](https://thunderstore.io/c/monster-train-2/p/GravitonGamer/MT2UnlimitedPurchases/) - Allows unlimited rerolls to make up for the diluted upgrade pool
- [Upgrade Champions](https://thunderstore.io/c/monster-train-2/p/GravitonGamer/Upgrade_Champions_2/) - Try the new upgrade stones on your champion

If you loved this or any of my other mods, feel free to [tip me a coffee.](https://ko-fi.com/gravitongamer)

# Full List of New Upgrade Stones

| Name         | Type  | Clan            | Rarity   | Effect                                                 |
| ------------ | ----- | --------------- | -------- | ------------------------------------------------------ |
| Bloomstone   | Unit  | Awoken          | Rare     | Add Rejuvenate: +4 Attack, +4 Health                   |
| Healstone    | Spell | Awoken          | Common   | Restore 5 HP to all allies.                            |
| Sunstone     | Spell | Awoken          | Common   | Draw +1 next turn.                                     |
| Vinestone    | Spell | Awoken          | Uncommon | Apply Rooted 1 to all enemies.                         |
| Cloudstone   | Unit  | Banished        | Uncommon | Add Shift: +2 Attack, Armor 2                          |
| Revengestone | Unit  | Banished        | Uncommon | Add Revenge: +4 Attack, Armor 4                        |
| Weakstone    | Spell | Banished        | Rare     | Apply Melee Weakness 1 to all enemies.                 |
| Wingstone    | Unit  | Banished        | Rare     | Add Ability: Flight                                    |
| Backstone    | Spell | Clanless        | Uncommon | Add Retreat. (Targeted spells only.)                   |
| Bannerstone  | Unit  | Clanless        | Common   | Add Deployable. Attack +10.                            |
| Boomstone    | Spell | Clanless        | Rare     | Add Explosive. Spell Power +20.                        |
| Curestone    | Spell | Clanless        | Rare     | Remove all debuff effects from friendly units.         |
| Downstone    | Spell | Clanless        | Uncommon | Add Descend. (Targeted spells only.)                   |
| Extremestone | Spell | Clanless        | Rare     | Spell Power +30.                                       |
| Frontstone   | Spell | Clanless        | Uncommon | Add Advance. (Targeted spells only.)                   |
| Scourstone   | Spell | Clanless        | Rare     | Remove all buff effects from enemy units.              |
| Shellstone   | Unit  | Clanless        | Uncommon | Add Titanskin 3.                                       |
| Snipestone   | Unit  | Clanless        | Common   | Add Sniper. +10 attack.                                |
| Stompstone   | Unit  | Clanless        | Rare     | Add Trample.                                           |
| Swipestone   | Unit  | Clanless        | Rare     | Add Sweep. Attack -20.                                 |
| Trainstone   | Unit  | Clanless        | Common   | Ember cost is 0.                                       |
| Truestone    | Spell | Clanless        | Uncommon | Add Piercing. Spell Power +10.                         |
| Tunestone    | Spell | Clanless        | Rare     | Add Attuned.                                           |
| Upstone      | Spell | Clanless        | Uncommon | Add Ascend. (Targeted spells only.)                    |
| Valuestone   | Spell | Clanless        | Rare     | Ember cost -3.                                         |
| Vilestone    | Spell | Clanless        | Rare     | Apply Corruption 20 to all enemies.                    |
| Apexstone    | Unit  | Hellhorned      | Rare     | Add Armored: +4 Attack.                                |
| Armorstone   | Unit  | Hellhorned      | Common   | Add Armor 25.                                          |
| Impstone     | Spell | Hellhorned      | Uncommon | Add a common or uncommon Imp unit to your hand.        |
| Strikestone  | Unit  | Hellhorned      | Uncommon | Add Strike: +4 Attack, Armor 4                         |
| Artistone    | Unit  | Lazarus League  | Rare     | Add Artificer: +8 Attack, Armor 8                      |
| Flaskstone   | Spell | Lazarus League  | Uncommon | Add Mix.                                               |
| Madstone     | Unit  | Lazarus League  | Rare     | Add Unstable 20.                                       |
| Radstone     | Spell | Lazarus League  | Uncommon | Apply Unstable 10 to all enemies.                      |
| Bladestone   | Unit  | Luna Coven      | Rare     | Add Mageblade 2.                                       |
| Cyclestone   | Unit  | Luna Coven      | Uncommon | Add Mooncycle: +2 Attack, +2 Health                    |
| Feeblestone  | Spell | Luna Coven      | Rare     | Apply Spell Weakness 1 to all enemies.                 |
| Lunestone    | Spell | Luna Coven      | Common   | Add Phase. Ember cost -1.                              |
| Dazestone    | Spell | Melting Remnant | Rare     | Apply Dazed 1 to all enemies.                          |
| Formstone    | Spell | Melting Remnant | Uncommon | Reform a random unit.                                  |
| Gravestone   | Unit  | Melting Remnant | Rare     | Add Harvest: +2 Attack, +2 Health                      |
| Smokestone   | Unit  | Melting Remnant | Common   | Add Stealth 2. +10 attack.                             |
| Brimstone    | Spell | Pyreborne       | Common   | Deal 3 Damage to all enemies.                          |
| Hoardstone   | Spell | Pyreborne       | Rare     | Gain 1 Dragon's Hoard.                                 |
| Lootstone    | Spell | Pyreborne       | Uncommon | Gain 10 Gold.                                          |
| Pyrestone    | Spell | Pyreborne       | Uncommon | Apply Pyregel 5 to all enemies.                        |
| Forgestone   | Spell | Railforged      | Rare     | Add Forge 5.                                           |
| Hearthstone  | Spell | Railforged      | Common   | Add Refined 3.                                         |
| Smeltstone   | Spell | Railforged      | Uncommon | Smelt a card in hand.                                  |
| Steelstone   | Unit  | Railforged      | Rare     | Add Steelguard and Armor 50.                           |
| Froststone   | Spell | Stygian Guard   | Uncommon | Apply Frostbite 10 to all enemies.                     |
| Meekstone    | Spell | Stygian Guard   | Uncommon | Apply Sap 2 to all enemies.                            |
| Mutestone    | Spell | Stygian Guard   | Uncommon | Apply Mute 1 to all enemies.                           |
| Offerstone   | Spell | Stygian Guard   | Common   | Add Offering. Ember cost -1. (Targetless spells only.) |
| Gorgestone   | Unit  | Umbra           | Rare     | Add Gorge: +2 Attack, +2 Health                        |
| Hungerstone  | Unit  | Umbra           | Rare     | Add Hunger.                                            |
| Leechstone   | Unit  | Umbra           | Common   | Add Lifesteal 2. +10 Attack.                           |
| Morselstone  | Spell | Umbra           | Uncommon | Add a common or uncommon Morsel unit to your hand.     |
| Growstone    | Spell | Underlegion     | Uncommon | Propagate 1 on all units.                              |
| Plaguestone  | Spell | Underlegion     | Uncommon | Apply Decay 4 to all enemies.                          |
| Rallystone   | Unit  | Underlegion     | Rare     | Add Rally: +2 Attack, +2 Health                        |
| Spawnstone   | Spell | Underlegion     | Uncommon | Add Spawn 1.                                           |
| Etchstone    | Unit  | Wurmkin         | Rare     | Add Etch: +4 Attack, +4 Health                         |
| Fusionstone  | Spell | Wurmkin         | Common   | Gain 1 Charged Echo.                                   |
| Inspirestone | Unit  | Wurmkin         | Rare     | Add Inspire: +2 Attack, +2 Health                      |
| Reapstone    | Spell | Wurmkin         | Uncommon | Apply Reap 5 to all enemies.                           |

# Enhanced Stones for Soul Savior Mode

| Name              | Type  | Clan            | Rarity   | Effect                                                 |
| ----------------- | ----- | --------------- | -------- | ------------------------------------------------------ |
| Bloomslab         | Unit  | Awoken          | Rare     | Add Rejuvenate: +8 Attack, +8 Health                   |
| Healstone+        | Spell | Awoken          | Common   | Restore 10 HP to all allies.                           |
| Sunstone+         | Spell | Awoken          | Common   | Draw +2 next turn.                                     |
| Vinestone+        | Spell | Awoken          | Uncommon | Apply Rooted 2 to all enemies.                         |
| Cloudslab         | Unit  | Banished        | Uncommon | Add Shift: +4 Attack, Armor 4                          |
| Revengeslab       | Unit  | Banished        | Uncommon | Add Revenge: +8 Attack, Armor 8                        |
| Weakstone+        | Spell | Banished        | Rare     | Apply Melee Weakness 2 to all enemies.                 |
| Wingslab          | Unit  | Banished        | Rare     | Add Ability: Flight. Health +30.                       |
| Backstone+        | Spell | Clanless        | Uncommon | Add Retreat. Ember cost -1. (Targeted spells only.)    |
| Bannerslab        | Unit  | Clanless        | Common   | Add Deployable. Attack +30.                            |
| Boomstone+        | Spell | Clanless        | Rare     | Add Explosive. Spell Power +40.                        |
| Curestone (Same)  | Spell | Clanless        | Rare     | Remove all debuff effects from friendly units.         |
| Downstone+        | Spell | Clanless        | Uncommon | Add Descend. Ember cost -1. (Targeted spells only.)    |
| Extremestone+     | Spell | Clanless        | Rare     | Spell Power +60.                                       |
| Frontstone+       | Spell | Clanless        | Uncommon | Add Advance. Ember cost -1. (Targeted spells only.)    |
| Scourstone (Same) | Spell | Clanless        | Rare     | Remove all buff effects from enemy units.              |
| Shellslab         | Unit  | Clanless        | Uncommon | Add Titanskin 7.                                       |
| Snipeslab         | Unit  | Clanless        | Common   | Add Sniper. +30 attack.                                |
| Stompslab         | Unit  | Clanless        | Rare     | Add Trample. Attack +20.                               |
| Swipeslab         | Unit  | Clanless        | Rare     | Add Sweep.                                             |
| Trainslab         | Unit  | Clanless        | Common   | Ember cost is 0. Attack +20.                           |
| Truestone+        | Spell | Clanless        | Uncommon | Add Piercing. Spell Power +30.                         |
| Tunestone+        | Spell | Clanless        | Rare     | Add Attuned. Spell Power +5.                           |
| Upstone+          | Spell | Clanless        | Uncommon | Add Ascend. Ember cost -1. (Targeted spells only.)     |
| Valuestone+       | Spell | Clanless        | Rare     | Ember cost -6.                                         |
| Vilestone+        | Spell | Clanless        | Rare     | Apply Corruption 40 to all enemies.                    |
| Apexslab          | Unit  | Hellhorned      | Rare     | Add Armored: +8 Attack.                                |
| Armorslab         | Unit  | Hellhorned      | Common   | Add Armor 50.                                          |
| Impstone+         | Spell | Hellhorned      | Uncommon | Add an Imp unit to your hand.                          |
| Strikeslab        | Unit  | Hellhorned      | Uncommon | Add Strike: +8 Attack, Armor 8                         |
| Artislab          | Unit  | Lazarus League  | Rare     | Add Artificer: +12 Attack, Armor 12                    |
| Flaskstone+       | Spell | Lazarus League  | Uncommon | Add Mix Twice.                                         |
| Madslab           | Unit  | Lazarus League  | Rare     | Add Unstable 40.                                       |
| Radstone+         | Spell | Lazarus League  | Uncommon | Apply Unstable 20 to all enemies.                      |
| Bladeslab         | Unit  | Luna Coven      | Rare     | Add Mageblade 4.                                       |
| Cycleslab         | Unit  | Luna Coven      | Uncommon | Add Mooncycle: +4 Attack, +4 Health                    |
| Feeblestone+      | Spell | Luna Coven      | Rare     | Apply Spell Weakness 2 to all enemies.                 |
| Lunestone+        | Spell | Luna Coven      | Common   | Add Phase. Ember cost -2.                              |
| Dazestone+        | Spell | Melting Remnant | Rare     | Apply Dazed 2 to all enemies.                          |
| Formstone+        | Spell | Melting Remnant | Uncommon | Reform 2 random units.                                 |
| Graveslab         | Unit  | Melting Remnant | Rare     | Add Harvest: +4 Attack, +4 Health                      |
| Smokeslab         | Unit  | Melting Remnant | Common   | Add Stealth 4. +30 attack.                             |
| Brimstone+        | Spell | Pyreborne       | Common   | Deal 6 Damage to all enemies.                          |
| Hoardstone+       | Spell | Pyreborne       | Rare     | Gain 2 Dragon's Hoard.                                 |
| Lootstone+        | Spell | Pyreborne       | Uncommon | Gain 20 Gold.                                          |
| Pyrestone+        | Spell | Pyreborne       | Uncommon | Apply Pyregel 10 to all enemies.                       |
| Forgestone+       | Spell | Railforged      | Rare     | Add Forge 10.                                          |
| Hearthstone+      | Spell | Railforged      | Common   | Add Refined 6.                                         |
| Smeltstone+       | Spell | Railforged      | Uncommon | Draw 1. Smelt a card in hand.                          |
| Steelslab         | Unit  | Railforged      | Rare     | Add Steelguard and Armor 75.                           |
| Froststone+       | Spell | Stygian Guard   | Uncommon | Apply Frostbite 20 to all enemies.                     |
| Meekstone+        | Spell | Stygian Guard   | Uncommon | Apply Sap 4 to all enemies.                            |
| Mutestone+        | Spell | Stygian Guard   | Uncommon | Apply Mute 2 to all enemies.                           |
| Offerstone+       | Spell | Stygian Guard   | Common   | Add Offering. Ember cost -2. (Targetless spells only.) |
| Gorgeslab         | Unit  | Umbra           | Rare     | Add Gorge: +4 Attack, +4 Health                        |
| Hungerslab        | Unit  | Umbra           | Rare     | Add Hunger. Health +30.                                |
| Leechslab         | Unit  | Umbra           | Common   | Add Lifesteal 4. +30 Attack.                           |
| Morselstone+      | Spell | Umbra           | Uncommon | Add a Morsel unit to your hand.                        |
| Growstone+        | Spell | Underlegion     | Uncommon | Propagate 2 on all units.                              |
| Plaguestone+      | Spell | Underlegion     | Uncommon | Apply Decay 8 to all enemies.                          |
| Rallyslab         | Unit  | Underlegion     | Rare     | Add Rally: +4 Attack, +4 Health                        |
| Spawnstone+       | Spell | Underlegion     | Uncommon | Add Spawn 2.                                           |
| Etchslab          | Unit  | Wurmkin         | Rare     | Add Etch: +8 Attack, +8 Health                         |
| Fusionstone+      | Spell | Wurmkin         | Common   | Gain 2 Charged Echoes.                                 |
| Inspireslab       | Unit  | Wurmkin         | Rare     | Add Inspire: +4 Attack, +4 Health                      |
| Reapstone+        | Spell | Wurmkin         | Uncommon | Apply Reap 10 to all enemies.                          |
